...
import { fetchUsers, fetchListarVotos } from '../actions/userActions';
...
const MaisVotos: React.FC = () => {
      ...
      if (senha.length === 6) {
        dispatch(loginUser({ matricula: `${userInfo?.matricula}`, senha }))
          .then(() => {
            ...
            setLoginError('');
            handlePasswordReset();
            resetVotesWithPassword('Lk2kbkb')(dispatch);
            dispatch(fetchUsers());
            dispatch(fetchListarVotos());
            navigate('/votar');
            alert('Votos Aplicados');
  ...
  return (
    <>{nivel === 'gerencia' || userInfo?.matricula === 970016 ? <div className='painel-controlador-votos'>
      <h1>Adicionar +1 Voto para Todos?</h1>
