                ...
                <div className='tela-painel-votos'>
                    {top3Pessoas.map((pessoa) => (
                        <div key={pessoa.id} onClick={() => handleVotoChange(pessoa.id)}>
                            <label className={`radio-label ${pessoa.id === voto ? 'radio-label-selecionado' : 'radio-label-nao-selecionado'} ${pessoa.id === voto && showConfirmDialog ? 'radio-label-voto' : 'radio-label-voto-nao'}`} htmlFor={`radio-${pessoa.id}`} style={{ display: `${pessoa.id === usuarioLogado ? 'none' : ''}` }}>
